...
onMounted(async () => {
  subcategories.value = await mockApi.getSubcategories();
});
  ...
  <header class="relative px-6 py-20 bg-slate-900 overflow-hidden text-center text-white rounded-b-[3rem]">
    ...
    <div class="relative max-w-3xl mx-auto space-y-8">
      <h1 class="text-4xl md:text-6xl font-black tracking-tight leading-tight">
        The Expert for your <span class="text-[#C1ED00]">Every Project</span>
      </h1>
      ...
      <div class="max-w-xl mx-auto relative group">
        <div class="absolute inset-y-0 left-5 flex items-center pointer-events-none">
          <CommonIcon name="search" size="20" class="text-slate-400 group-focus-within:text-[#C1ED00] transition-colors" />
        </div>
        <input
          type="text"
          v-model="query"
          placeholder="Search any sub categories (e.g. plumbing, haircut)..."
          class="w-full pl-14 pr-6 py-5 bg-white text-slate-900 rounded-2xl shadow-2xl focus:outline-none focus:ring-4 focus:ring-[#C1ED00]/20 transition-all text-lg font-medium"
        ...
        >
          <button
            ...
            class="w-full px-6 py-4 flex items-center gap-4 hover:bg-gray-50 transition-colors text-left border-b last:border-none border-gray-50"
          >
            <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden">
               <img :src="resolveImagePath(sub.image)" class="w-full h-full object-cover" @error="(e) => (e.target.src = '/images/default.jpg')"/>
            </div>
            <div>
              <p class="font-bold text-slate-900">{{ sub.name }}</p>
              <p class="text-xs text-gray-400 uppercase tracking-widest font-bold">In {{ sub.categoryName }}</p>
            </div>
          </button>
        </div>
      </div>
