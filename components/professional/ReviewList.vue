<template>
  <div class="space-y-8">
    ...
    <div v-else class="space-y-6">
      <div  v-for="review in reviews"  :key="review.id" class="p-6 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
...
const fetchReviews = async () => {
  loading.value = true;
  try {
    const allReviews = await $fetch('/mock-data/reviews.json').catch(() => []);
    reviews.value = allReviews.filter(r => r.professionalId === props.professionalId);
  } catch (err) {
    reviews.value = [];
  } finally {
    loading.value = false;
